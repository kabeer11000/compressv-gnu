...
/*
Compilation command:
riscv64-linux-gnu-gcc -march=rv64gcv -mabi=lp64d -static -o vector_test hello_vector.c

Key changes made:
1. Changed from double to int64_t for better toolchain compatibility
2. Added __riscv_ prefix to intrinsic functions (newer naming convention)
3. Used integer vector types and operations instead of floating-point
4. Updated printf format specifiers to %ld for int64_t
*/